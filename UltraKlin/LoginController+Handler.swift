...
extension UltraKlinLogin: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    
    func handelRegister() {
        guard let name = (UserDefaults.standard.string(forKey: "userName")), let email = textLoginUsername.text, let password = textLoginPassword.text else{
            return
        }
        let nameChace = (UserDefaults.standard.string(forKey: "userName"))
        print(nameChace as Any)
        print(textLoginUsername.text as Any)
        print(textLoginPassword.text as Any)
...
//            self.messagesController?.navigationController?.title = values["name"] as? String
    ...
    func handelLogIn() {
        guard let email = textLoginUsername.text, let password = textLoginPassword.text else {
            return
        }
        
        Auth.auth().signIn(withEmail: email, password: password, completion: { (user, error) in
            if error != nil {
                print("Account not login, change for register account.")
                self.handelRegister()
            }
