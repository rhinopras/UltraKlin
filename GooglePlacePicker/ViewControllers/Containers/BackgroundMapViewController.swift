...
class BackgroundMapViewController: UIViewController, CAAnimationDelegate {
  ...
  var coordinate = CLLocationCoordinate2D(latitude: -6.2296829, longitude: 106.7729614) {
    didSet {
      updateCoordinate()
    }
  }
  ...
  private func startAnimatingMap() {
    ...
    let targetCoordinate = CLLocationCoordinate2D(latitude: coordinate.latitude + randomOffset(), longitude: coordinate.longitude + randomOffset())
