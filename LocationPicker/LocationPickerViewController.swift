    ...
    func selectLocation(location: CLLocation) {
        ...
        geocoder.reverseGeocodeLocation(location) { response, error in
            if let error = error as NSError?, error.code != 10 { // ignore cancelGeocode errors
                ...
                let alert = UIAlertController(title: nil, message: "Please check your connection internet and try again. \n " + error.localizedDescription, preferredStyle: .alert)
                ...
                var name = placemark.areasOfInterest?.first
...
extension LocationPickerViewController: MKMapViewDelegate {
	...
	func selectLocationButton() -> UIButton {
        ...
        if let titleLabel = button.titleLabel {
            let width = titleLabel.textRect(forBounds: CGRect(x: 0, y: 0, width: Int.max, height: 30), limitedToNumberOfLines: 3).width
